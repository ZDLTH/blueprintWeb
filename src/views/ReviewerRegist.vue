  ...
  methods: {
    onSubmit() {
      ...
      if(this.userType === 3){
        ...
        data = {user:{"logid":logid, "password":this.form.password, "tid":"4"}, reviewdevelopment:{"idNumber":this.form.idNumber,"name":this.form.name, "userType":0}}
      }else {
        logid = this.form.creditCode;
        data = {user:{"logid":logid, "password":this.form.password, "tid":"4"}, reviewdevelopment:{"creditCode":this.form.creditCode,"companyName":this.form.companyName, "userType":1}}
      }

      this.$axios.post("/reviewdevelopment/regist",data)
          .then(res=>{
            alert(res.data["msg"])
          })
    }
