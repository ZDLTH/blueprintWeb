  ...
  methods: {
    onSubmit(formName) {
      ...
      if(this.userType === 3){
        ...
        data = {user:{"logid":logid, "password":this.form.password, "tid":"2"}, programmedesigner:{"idNumber":this.form.idNumber,"name":this.form.name, "userType":0}}
      }else {
        logid = this.form.creditCode;
        data = {user:{"logid":logid, "password":this.form.password, "tid":"2"}, programmedesigner:{"creditCode":this.form.creditCode,"companyName":this.form.companyName, "userType":1}}
      }

      // this.$axios.post("/programmedesigner/regist",data)
      // .then(res=>{
      //     alert(res.data["msg"])
      // })
    }
